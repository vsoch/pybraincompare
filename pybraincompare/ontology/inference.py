...
def _calculate_reverse_inference_vectors_ranges(mrtable,p_in,p_out,in_count,out_count,range_table):
    '''
    Internal function to return two strings of probabilities: one for probabilities from likelihood table for voxels with activations [p_in_vector], and the other to return probabilities for voxels without activation [p_out_vector]. These probabilities can go into a reverse inference calculation for calculating a score based on binarizing an image. 
    '''
...
def _calculate_reverse_inference_vectors_binary(mrtable,p_in,p_out,in_count,out_count):
    '''Internal function to return two strings of probabilities: one for probabilities from likelihood table derived from images relevant to a contrast, and the other from a likelihood table from images not relevant - both are limited to voxels with activation for the image, defined as above/below the threshold defined in the columns of the likelihood table. This will be use for reverse inference calculation of a single image based on an absolute threshold to define activation. 

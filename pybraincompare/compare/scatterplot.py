...
from .mrutils import (
    ...
    make_binary_deletion_vector,
    resample_images_ref
)
from pybraincompare.template.futils import (
    get_package_dir,
    make_tmp_folder, 
    make_dir,
    unzip
...
def scatterplot_canvas(image_vector1,
                       image_vector2,
                       image_names,
                       atlas_vector,
                       atlas_labels,
    ...
    ''' Generate a canvas (whole brain) scatterplot, meaning we render ALL the 
    data and the browser does not explode. Currently only using images 
    registered to mni 2mm template is supported, as the atlas is part of the 
    page template, and currently only option is to provide output directory
    '''
    ...
    data_table = calculate_atlas_correlation(image_vector1,
                                             image_vector2,
                                             atlas_vector,
                                             atlas_labels,
                                             atlas_colors,
    ...
    data_table_summary = calculate_atlas_correlation(image_vector1,
                                                     image_vector2,
                                                     atlas_vector,
                                                     atlas_labels,
                                                     atlas_colors,
    ...
    for region in data_table_summary["labels"]:
        ...
        outfile = "%s/%s_data.tsv" %(data_directory,region.replace('"',''))
    ...
    outfile =  "%s/scatterplot_data.tsv" %(data_directory)
    data_table.to_csv(outfile, sep="\t",index=False)
...
def scatterplot_compare_vector(image_vector1,
                               image_vector2,
                               image_names,
                               atlas_vector,
                               atlas_labels,
                               ...
                               corr_type="pearson",
                               atlas=None,
                               subsample_every=None,
                               remove_scripts=None,
                               summary=False,
                               width=1200):

    '''scatterplot_compare_vector: Generate a d3 scatterplot with all arguments
    as vectors to outputs html with the generated d3 plot. If atlas==None, 
    no atlas will be rendered on the page. If you want to speed up page 
    performance, it is recommended to generate the atlas svg ahead of time,
    and embed in your page.
    '''
    ...
    if len(image_vector1) == len(image_vector2) == len(atlas_vector) == len(atlas_labels) == len(atlas_colors):
        ...
        if len(np.where(pdmask!=0)[0]) > 3:    
            ...
            if subsample_every != None:
                sample_index = np.arange(0,len(image_vector1),
                                           int(subsample_every))
            ...
            corrs_df = calculate_atlas_correlation(image_vector1,
                                                   image_vector2,
                                                   atlas_vector,
                                                   atlas_labels,
                                                   atlas_colors,
    ...
    if error != None:
        corrs_df = pandas.DataFrame(columns=["INPUT_DATA_ONE",
                                             "INPUT_DATA_TWO",
                                             "ATLAS_DATA",
                                             "ATLAS_LABELS",
                                             "ATLAS_CORR",
    ...
    template = make_scatterplot_interface(corrs_df,
                                          error=error,
                                          elements=elements,
...
def scatterplot_compare(images,
                        image_names,
                        software="FSL",
                        atlas=None,
                        atlas_rendering=None,
                        custom=None,
                        corr_type="pearson",
                        reference=None,
                        resample_dim=[8,8,8],
    ...
    - images: list with full paths to image 1, image 2, or nibabel nifti1 images. 
              Must be in MNI space [required]
    - software: FSL or FREESURFER [default FSL]
    - atlas: a pybraincompare "atlas" object, will be rendered in vis and color 
             data points [default None]
    - atlas_rendering: a pybraincompare "atlas" object for rending svg (should 
                       be higher res, 2mm) [default None]
    - custom: custom dictionary of {"TEMPLATE_IDS":,"text"} [default None]
    - corr: regional correlation type to include [default pearson]
    - reference: if a different standard mask is desired to resample 
                 images to [default None]
    - resample_dim: the dimension to resample the reference, and then images to
    - remove_scripts: will remove resources from template
    '''
    ...
    images_resamp, _ = resample_images_ref(images=images_nii,
                                           reference=reference,
    ...
    pdmask = nibabel.Nifti1Image(pdmask,
                                 header=images_resamp[0].get_header(),
    ...
    if not (pdmask.get_data() == 0).all():
        corr_df = calculate_correlation(images=images_resamp,
                                        mask=pdmask,
                                        atlas=atlas,
                                        corr_type=corr_type)

        # If nan, no surviving regional correlations 
    ...
    # If we have an error, generate empty dataframe
    if error != None:
        corr_df = pandas.DataFrame(columns=["INPUT_DATA_ONE",
                                            "INPUT_DATA_TWO",
                                            "ATLAS_DATA",
                                            "ATLAS_LABELS",
                                            "ATLAS_CORR",
    ...
    # Prepare template from correlation data frame
    template = make_scatterplot_interface(corr_df,
                                          error=error,
                                          elements=elements,
                                          remove_scripts=remove_scripts)
...
def get_atlas_objects(atlas,atlas_rendering):
    '''return atlas for rendering (2mm) and atlas for applying to images (atlas)
    based on what is provided by the user:
    - if atlas not specified, use MNI 152 2mm for visual, 8mm for regions
    - if atlas specified, but not a rendering, use atlas as the rendering
    ...
    if atlas == None:
        ...
        # we render whatever the user provided
        if atlas_rendering == None: atlas2mm = atlas
